...
import { decodeProjectSlug } from "../paths.ts";
...
import type { Provider, SessionMeta } from "./types.ts";

const CLAUDE_PROJECTS_DIR_PATH = CLAUDE_PROJECTS_DIR;
...
async function indexSessionFile(filePath: string): Promise<SessionMeta> {
...
export const claudeProvider: Provider = {
  ...
  async listProjects(): Promise<ProjectInfo[]> {
    let entries;
    try {
      entries = await readdir(CLAUDE_PROJECTS_DIR_PATH, { withFileTypes: true });
    } catch {
      return [];
    }
    const dirs = entries.filter((e) => e.isDirectory());
    const projects = await mapLimit(dirs, IO_CONCURRENCY, async (e): Promise<ProjectInfo | null> => {
      const dirPath = join(CLAUDE_PROJECTS_DIR_PATH, e.name);
      let files: string[];
      try {
  ...
  async listSessions(projectSlug: string): Promise<SessionListItem[]> {
    const dirPath = join(CLAUDE_PROJECTS_DIR_PATH, projectSlug);
    let entries: string[];
    try {
      entries = (await readdir(dirPath)).filter((f) => f.endsWith(".jsonl"));
    } catch {
      return [];
    }
  ...
  async findSessionById(sessionId: string): Promise<string | null> {
    ...
    for (const p of entries) {
      ...
      const candidate = join(CLAUDE_PROJECTS_DIR_PATH, p.name, `${sessionId}.jsonl`);
      try {
  ...
  async indexSessionFile(filePath: string): Promise<SessionMeta> {
    return indexSessionFile(filePath);
  },
